...
U8 ** Enc(AES_KEY *k, int bits, U8 *m)
{
	...
	U8 *F_kr = (U8 *)calloc(bytes, sizeof(U8));
	AES_encrypt(c[0], F_kr, k);
	
	//type casting U8 F_k(r)-> BN F_k(r)    for F_k(r) xor m
	BIGNUM *BN_F_kr = BN_new();
	BN_bin2bn(F_kr, strlen(F_kr), BN_F_kr);
	...
	//C2 = F_k(r) xor m
	BIGNUM *BN_C2 = BN_new();
	BN_xor(BN_C2, bits, BN_F_kr, BN_m);
...
U8 *Dec(AES_KEY *k, int bits, U8 **C)
{
	...
	BIGNUM *BN_m = BN_new();
	BN_xor(BN_m, bits, BN_F_k_C1, BN_C2);
	BN_bn2bin(BN_m, M);
